...
(function() {
  ...
  {
    "text": "A DBA needs to configure a firewall rule that blocks a specific IP address from accessing only one database on an Azure SQL server. Which approach should they use?",
    "options": [
    ...
    "correct": 1,
    "module": 2,
    "explanation": "Database-level firewall rules can grant or block specific IP addresses from accessing specific databases. They are configured through T-SQL only using the `sp_set_database_firewall_rule` stored procedure from within the user database. Server-level rules apply to all databases."
  },
  {
  ...
  // === Defender for SQL, Ledger & Purview ===
  {
  ...
  if (window.__dp300.modules.length < 2) {
    window.__dp300.modules[1] = "Module 2: Authentication, Authorization, Encryption & Compliance";
  }
})();
